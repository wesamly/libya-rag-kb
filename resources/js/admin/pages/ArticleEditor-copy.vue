        ...
        <div class="card h-100">
          <div class="card-body d-flex flex-column">
            ...
            <div class="flex-grow-1" style="min-height: 400px; display: flex; flex-direction: column;">
              <!-- 
                THE FIX (Part 1): 
                - We bind v-model:content to our dedicated 'quillContent' ref.
                - We bind :key to 'editorKey' to force a re-mount after data loads.
              -->
              <QuillEditor
                :key="editorKey" 
...
const fetchData = async () => {
  ...
  try {
    ...
    if (props.id) {
      ...
      // Update the main article ref
      article.value.title = fetchedArticle.title;
      article.value.category_id = fetchedArticle.category_id;
      article.value.status = fetchedArticle.status;
      article.value.tags = fetchedArticle.tags.map(tag => ({ value: tag.id, label: tag.name }));
      
      // Explicitly set the separate quillContent ref
      quillContent.value = fetchedArticle.content || '';
      
    } else {
      // We are creating a new article
      article.value.title = route.query.title || ''; // Pre-fill from content gap
      // Reset all other fields
      quillContent.value = ''; // Reset content
    ...
    loading.value = false;
    // --- THE FIX (Part 2) ---
    // Increment the key. This forces Vue to destroy the old
    // <QuillEditor> and create a new one, which will now
    // read the correct initial value from 'quillContent'.
    editorKey.value++;
    // ------------------------
  }
};
...
const saveArticle = async (statusOverride = null) => {
  ...
  const payload = {
    ...article.value,
    content: quillContent.value, // <-- Use the content from the quillContent ref
  ...
  try {
    ...
    if (!props.id) {
      ...
      // Update quillContent ref (in case it was modified on save)
      quillContent.value = updatedArticle.content || ''; 
